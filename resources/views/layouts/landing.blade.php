...
<head>
    ...
    @php
        $resolvedMediaAssets = \Illuminate\Support\Facades\Schema::hasTable('media_assets')
            ...
            : collect();
        $landingFavicon = $resolvedMediaAssets->get('brand_favicon') ?: $resolvedMediaAssets->get('brand_logo');
    @endphp
    @if($landingFavicon?->url())
        <link rel="icon" type="image/png" href="{{ $landingFavicon->url() }}">
        <link rel="shortcut icon" href="{{ $landingFavicon->url() }}">
    @endif
    @php
