    ...
    <div class="flex min-h-screen items-center justify-center bg-slate-950 px-4">
        <form method="POST" action="{{ route('superadmin.login.store') }}" class="w-full max-w-md rounded-3xl border border-slate-800 bg-slate-900 p-8 shadow-2xl" x-data="{ showPassword: false }">
            ...
            <div class="mt-6 space-y-4">
                <div><label class="mb-2 block text-sm text-slate-300">Username</label><input name="username" value="{{ old('username') }}" autocomplete="username" autofocus class="w-full rounded-2xl border border-slate-700 bg-slate-950 px-4 py-3 text-white" required></div>
                <div>
                    <label class="mb-2 block text-sm text-slate-300">Parol</label>
                    <div class="relative">
                        <input name="password" x-bind:type="showPassword ? 'text' : 'password'" autocomplete="current-password" class="w-full rounded-2xl border border-slate-700 bg-slate-950 px-4 py-3 pr-12 text-white" required>
