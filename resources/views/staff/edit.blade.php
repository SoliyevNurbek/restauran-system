<x-app-layout title="Xodimni tahrirlash" pageTitle="Xodimni tahrirlash">
    <div class="rounded-2xl bg-white p-6 shadow-soft dark:bg-slate-900">
        <form action="{{ route('employees.update', $employee) }}" method="POST" class="grid gap-4 md:grid-cols-2" data-loading-form>
            @csrf @method('PUT')
            @include('staff.form', ['employee' => $employee, 'roles' => $roles])
