        ...
        <table class="min-w-full text-sm">
            ...
            @forelse($costs as $cost)
                <tr class="border-t border-slate-100 dark:border-slate-800">
                    ...
                    <td class="px-4 py-3"><div class="flex justify-end gap-2"><a href="{{ route('expenses.kitchen.edit', $cost) }}" class="rounded-lg border border-slate-200 px-3 py-1.5 text-xs dark:border-slate-700">Tahrirlash</a><form action="{{ route('expenses.kitchen.destroy', $cost) }}" method="POST">@csrf @method('DELETE')<x-delete-button /></form></div></td>
                </tr>
            @empty
